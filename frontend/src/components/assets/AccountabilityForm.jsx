...
export default function AccountabilityForm({ serializedAssets, initialAssetId, handleApiCall, onSuccess }) {
  const [lines, setLines] = useState(() =>
    initialAssetId ? [{ serialized_asset_id: Number(initialAssetId), accessories_notes: '' }] : []
  );
  ...
  // Re-sync if the modal is reused for a different pre-selected asset without unmounting
  useEffect(() => {
    if (initialAssetId) {
      setLines([{ serialized_asset_id: Number(initialAssetId), accessories_notes: '' }]);
    }
  }, [initialAssetId]);
  ...
  const addLine = () => {
    ...
    setLines([...lines, { serialized_asset_id: Number(pendingAssetId), accessories_notes: '' }]);
    setPendingAssetId('');
  };
  ...
  const updateNotes = (assetId, notes) => {
    setLines(lines.map(l => l.serialized_asset_id === assetId ? { ...l, accessories_notes: notes } : l));
  };
      ...
      {lines.length > 0 && (
        <div className="form-group" style={{ border: '1px solid #eee', borderRadius: '6px', padding: '12px' }}>
          ...
          {lines.map(line => {
            ...
            const cost = Number(asset?.unit_cost ?? asset?.item?.unit_cost ?? 0);
            return (
              <div key={line.serialized_asset_id} style={{ marginTop: '10px', paddingBottom: '10px', borderBottom: '1px solid #f0f0f0' }}>
                ...
                <input
                  type="text"
                  placeholder="Bundled accessories (optional) — e.g. Includes keyboard SN:..., mouse SN:..."
                  value={line.accessories_notes}
                  onChange={e => updateNotes(line.serialized_asset_id, e.target.value)}
